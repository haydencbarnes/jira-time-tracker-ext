...
interface JiraErrorLike {
  message?: string;
}

/** Extracts a string message from thrown values (shared with callers that log errors). */
export function getErrorMessage(error: unknown): string {
  ...
  if (typeof error === 'object' && error !== null && 'message' in error) {
    const message = (error as JiraErrorLike).message;
...
export function handleJiraError(
  error: unknown,
  ...
  switch (statusCode) {
    case 400:
      ...
      if (context.includes('search') || context.includes('timer')) {
        actionableSteps =
          'The work item key or time format may be invalid. Verify the issue exists and use proper time format (e.g., 2h, 30m).';
      } else {
        actionableSteps =
          'Check your JQL query in Time Table settings (gear icon top right), ensure all field names are correct, and verify that referenced projects/issue types exist.';
      }
      break;

    case 401:
      errorMessage =
        'Authentication Failed - JIRA could not verify your identity';
      actionableSteps =
        "Your API token may be invalid or expired. Go to Settings and:\n1. Verify your username/email is correct\n2. Generate a new API token from your Atlassian account\n3. Ensure you're using an API token (not password) for Jira Cloud";
      break;
    ...
    case 500:
      ...
      actionableSteps =
        'This is a JIRA server issue. Try again in a few minutes, or contact your JIRA administrator if the problem persists.';
      break;

    case 503:
      errorMessage = 'JIRA Service Unavailable - Server is temporarily down';
      actionableSteps =
        'JIRA is temporarily unavailable. Wait a few minutes and try again.';
      break;
  ...
  const fullMessage = actionableSteps
    ? `${errorMessage}\n\n${actionableSteps}`
    : errorMessage;
...
const jiraErrorHandlerGlobal = globalThis as typeof globalThis & {
  JiraErrorHandler?: { handleJiraError: typeof handleJiraError };
};
