...
${actionableSteps}` : errorMessage;
  ...
  function attachIssueDirectHandlers(JIRA2, inputEl, getSelectedProjectKey, hooks, ctx) {
    if (!inputEl) return;
    const extractIssueKey = (raw) => typeof JIRA2?.extractIssueKey === "function" ? JIRA2.extractIssueKey(raw) : String(raw || "").trim().split(/\s|:/)[0].toUpperCase();
    const isIssueKeyLike = (key) => typeof JIRA2?.isIssueKeyLike === "function" ? JIRA2.isIssueKeyLike(key) : /^[A-Z][A-Z0-9_]*-\d+$/.test(key || "");
    const acceptIfValid = async () => {
      ...
      try {
        const { key, summary } = await JIRA2.resolveIssueKeyFast(
          candidate,
  ...
  function bindInfiniteIssuesScroll(issueList, issueItems, jql, JIRA2, issueInput, formatIssueRow, initialNextCursor) {
    ...
    issueList.addEventListener("scroll", () => {
      void (async () => {
        ...
        loadingMore = true;
        const nextPage = await JIRA2.getIssuesPage(jql, nextCursor, 100);
  ...
  async function setupProjectIssueAutocomplete(JIRA2, behavior) {
    ...
    const projectsResponse = await JIRA2.getProjects();
  ...
  // src/ts/timerFeatureModule/timer.ts
  function clearIssueStorageFromAutocomplete(ctx) {
    ...
    chrome.storage.sync.get(
      ...
      async (storedOptions) => {
        ...
        if (options.projectId && options.projectName) {
          getRequiredElement("projectId").value = `${options.projectId}: ${options.projectName}`;
        } else if (options.projectId) {
          getRequiredElement("projectId").value = options.projectId;
        }
        if (options.issueKey && options.issueTitle) {
          getRequiredElement("issueKey").value = `${options.issueKey}: ${options.issueTitle}`;
        } else if (options.issueKey) {
          getRequiredElement("issueKey").value = options.issueKey;
        }
        enforceProjectIssueConsistency();
        await init(options);
        getRequiredElement("startStop").addEventListener(
          "click",
          toggleTimer
        );
        getRequiredElement("reset").addEventListener(
          "click",
          resetTimer
        );
        getRequiredElement("logTime").addEventListener(
          "click",
          logTimeClick
        );
  ...
  async function setupAutocomplete(JIRA2) {
    const client = JIRA2;
      ...
      runInitialPreload: async (ctx) => {
        ...
        try {
          ...
          if (issueInputRef.current && issueInputRef.current.value) {
            const existingIssueKey = issueInputRef.current.value.split(":")[0].trim();
  ...
  function toggleTimer() {
    if (isRunning) {
      ...
      chrome.runtime.sendMessage({ action: "startTimer", seconds });
    }
    isRunning = !isRunning;
    applyTimerRunningUi();
    saveTimerState();
  ...
  function applyStoredTimerState(items) {
    ...
    if (isRunning && items.timerLastUpdated) {
      const elapsedSeconds = Math.floor(
        ((/* @__PURE__ */ new Date()).getTime() - items.timerLastUpdated) / 1e3
      );
  ...
  function getTimerChangeValue(changes, key, defaultValue, fallbackValue) {
    ...
    return Object.prototype.hasOwnProperty.call(changes[key], "newValue") ? changes[key].newValue : defaultValue;
  }
  ...
  async function logTimeClick() {
    ...
    try {
      const startedTime = (/* @__PURE__ */ new Date()).toISOString();
  ...
  function secondsToJiraFormat(totalSeconds) {
    ...
    if (seconds2 > 0) result += `${seconds2}s`;
    return result.trim();
  }
  function displayError(message) {
    const error = getRequiredElement("error");
    error.innerText = message;
    error.style.display = "block";
    getRequiredElement("success").style.display = "none";
  }
  function displaySuccess(message) {
    const success = getRequiredElement("success");
    success.innerText = message;
    success.style.display = "block";
    getRequiredElement("error").style.display = "none";
  }
  function saveTimerState() {
    chrome.storage.sync.set({
      timerSeconds: seconds,
      timerIsRunning: isRunning,
      timerLastUpdated: (/* @__PURE__ */ new Date()).getTime()
    });
  }
